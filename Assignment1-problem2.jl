...
# ╔═╡ 73b0f980-10d2-11f0-0734-8b46105f72a2
using PlutoUI

# ╔═╡ 69790c83-1d53-4f39-a95d-5de894c8b043
using CSV

# ╔═╡ ad014742-6b74-4f07-82f8-c55dd0b260fd
using DataFrames

# ╔═╡ 18087c25-5782-4084-ab1b-6a2520f591bf
using MLJ

# ╔═╡ f05b48a3-b476-4c47-9de6-1d471d638fe1
using Statistics

# ╔═╡ 791b6e84-5fbe-484c-9e32-24bf2e3118a9
using StatsBase

# ╔═╡ b4c26601-ae89-4e14-8c34-3fd9cbdf00f0
using AlgebraOfGraphics

# ╔═╡ 9652e9e8-f6b8-47b3-8a7e-0dcc32217760
using CairoMakie

# ╔═╡ 3affc530-e8cb-4c3d-aa3f-5fca1405ddec
using Random

# ╔═╡ 6c0a5b4d-24bf-491d-a3f1-64a9d1427635
using MLJLIBSVMInterface

# ╔═╡ e7b5ed14-d2d0-484e-8e33-e9d2f6ffc9c4
using LIBSVM

# ╔═╡ 5949c8e1-9550-41c2-b09b-23f814499920
using Makie

# ╔═╡ 5a1c56e8-36a8-4fd2-9936-2e495619d014
using EvoTrees

# ╔═╡ 28e83a1c-d8bd-4359-8d76-ef39d902fe10
using MLBase

# ╔═╡ 47869815-8e85-46c6-bedc-607ff444f5bb
using MLJLinearModels

# ╔═╡ 31afa900-2450-40e6-bc39-afbe57460056
PlutoUI.TableOfContents()

# ╔═╡ 72701436-7a54-48e8-93ec-9b5eb2e21c6e
TableOfContents(title="📚 Table of Contents", indent=true, depth=4, aside=true)

# ╔═╡ 2dba8e63-ff2a-4d6f-a925-e465b302f839
md"# Fetus Health Classifier"

# ╔═╡ caa55fdb-7e7d-46ed-a57c-e08c37707d4c
fetus_df = DataFrame(CSV.File("./dataset/fetal_health.csv",header=1))

# ╔═╡ 492273b1-0171-413b-943d-f9148f41a592
md"## 1. Data Exploration"

# ╔═╡ 61ce06e6-88ac-44ed-8cb2-1106b86974b2
scatter(fetus_df."baseline value", fetus_df.fetal_health, color=fetus_df.fetal_health)

# ╔═╡ f6ce5ef9-be6c-4886-a5d6-8dc2ee7dea38
describe(fetus_df)

# ╔═╡ 6a5cd853-6890-4755-a41f-4c7dadd74638
md"## 2. Data Preparation"

# ╔═╡ 192cee46-ff83-4153-92f2-17fb1545a358
coerce!(fetus_df,Count => Continuous)

# ╔═╡ b3c96051-37e1-462d-9494-6393dd959a64
coerce!(fetus_df,:fetal_health => OrderedFactor)

# ╔═╡ 1aae796b-6579-4feb-8193-9900cc99aa89
schema(fetus_df)

# ╔═╡ fb59293d-6432-4fef-8118-12eadbfdf77f
y,x_raw = unpack(fetus_df,==(:fetal_health),colname -> true)

# ╔═╡ 1643c137-f205-422e-a023-cbb83a54f989
x_fs_mach = machine(Standardizer(),x_raw)

# ╔═╡ 1faf747d-246c-4160-961f-f3c3b80953f4
x_norm = MLJ.transform(MLJ.fit!(x_fs_mach),x_raw)

# ╔═╡ 50254123-ae1b-4f27-86a6-3f0391b243c6
hotenc_mach = machine(OneHotEncoder(),x_norm) |> MLJ.fit!

# ╔═╡ 5c07faa6-db70-4177-a6b6-8b516b40fe40
x_fin = MLJ.transform(hotenc_mach,x_norm)

# ╔═╡ 176fe1f5-92b6-4c6a-881e-76e69028f14f
schema(x_fin) |> DataFrames.DataFrame

# ╔═╡ 42643fa6-f05a-472c-96cd-62e4b69edc1e
train_set,test_set = partition(eachindex(y),0.85;rng=Xoshiro(456))

# ╔═╡ 8426e98b-2ef9-407a-8d3d-49339ad5a43a
md"## 3. Model Selection"

# ╔═╡ cc0a250d-abd7-4137-ba87-b4de0b9b1fb0
for m in models(matching(x_fin,y))
	println(rpad(m.name,40),rpad(m.prediction_type,20),"($(m.package_name))")
end

# ╔═╡ fe9e8ef9-f482-4a9d-a3d6-684d2fa27a6d
md"## 4. Model Development: Booster Trees"

# ╔═╡ 0bc39780-af65-41c0-bbf3-4e3fe9599a19
# Loading EvoTree Model
EvoTreeModel = @load EvoTreeClassifier pkg=EvoTrees

# ╔═╡ 831e12fa-c3ff-40bd-8777-1057d5910e6b
# Creating EVoTree model Object
boostert_model = EvoTreeModel(tree_type="oblivious",rng=Xoshiro(456))

# ╔═╡ 5e9eb6b4-12e6-4297-927f-7b14dd6353e1
# Creating EvoTree model and encoding model features
piped_boostert_model = ContinuousEncoder() |> boostert_model

# ╔═╡ c8304350-ad1a-4933-ad46-04d6f46b333c
# Creating EvoTree machine
piped_boostert_mach = machine(piped_boostert_model,x_fin,y)

# ╔═╡ 3095464e-7a31-4bec-8919-7c28632a1ace
md"## 5. Model Training"

# ╔═╡ f6440ddd-da11-4ff1-b4d1-c6e8e6acc67f
MLJ.fit!(piped_boostert_mach,rows=train_set)

# ╔═╡ f113d834-5db0-40c1-a179-3f8fbc83bdbe
md"## 6. Model Testing"

# ╔═╡ 71b14414-8790-4765-9653-1ee6477971c0
yp_3 = MLJ.predict(piped_boostert_mach,rows=test_set)

# ╔═╡ 2dc1f43a-4c64-4cd5-9dd6-714aea6e27c4
md"## 7. Model Evaluation"

# ╔═╡ 5b61b1d8-f33f-4329-b36d-0e4f174011eb
# Creating model confusion matrix
cm_3 = confusion_matrix(mode.(yp_3),y[test_set])

# ╔═╡ cef4a211-a593-4727-ad90-7d85274354c6
...
manifest_format = "2.0"
project_hash = "60dd05b2525c6a0d86aa8cba345abc0498e4eb29"
...
git-tree-sha1 = "07e999cf2bb9e7aa07c751a8ab131e0900950fa0"
